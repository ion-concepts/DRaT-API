...
parser.add_argument('--module', type=str,
                    help=('Name of gr module containing file to bind '
                          '(e.g. fft digital analog)'))
...
parser.add_argument(
    '--defines', help='Set additional defines for precompiler',
    default=(), nargs='*')
parser.add_argument(
    '--include', help='Additional Include Dirs, separated',
    default=(), nargs='*')
...
with warnings.catch_warnings():
    ...
    bg = BindingGenerator(prefix, namespace,
                          prefix_include_root, output_dir,
                          define_symbols=defines,
                          addl_includes=includes,
                          catch_exceptions=False, write_json_output=False,
                          status_output=args.status,
                          flag_automatic=True
                          if args.flag_automatic.lower() in ['1', 'true']
                          else False,
                          flag_pygccxml=True
                          if args.flag_pygccxml.lower() in ['1', 'true']
                          else False)
